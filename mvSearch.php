...
$json = search($_GET['keywords'],$page,1004);
//$json = getdata($_GET['keywords'],$page);
$search = json_decode($json);
//print_r($search);
$number = $search->result->mvCount;//歌曲总数
$songCount =  count($search->result->mvs);//查询到的歌曲数

if($songCount == 0 | $songCount == false){
exit('<div class = \'error\'>错误:暂无你需要的MV</div>');
...
while($i<$songCount){
$n = $i+1;
$singer =  $search->result->mvs[$i]->artistName;//singer
if($singer == false){
echo <<<m
<div class ='search'>

<font color="#ff9966">{$n}</font>、<a href="mvInfo.php?id={$search->result->mvs[$i]->id}">{$search->result->mvs[$i]->name}</a></div>
...
echo <<<m
<div class ='search'>
<font color="#ff9966">{$n}</font>、<a href="mvInfo.php?id={$search->result->mvs[$i]->id}">{$search->result->mvs[$i]->name} - {$singer}</a></div>
m;
